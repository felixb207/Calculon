...
int connect_serial() {
    ...
    serialHandle = CreateFile(
        "\\\\.\\COM9", GENERIC_READ | GENERIC_WRITE,
...
void get_user_input(char *inputBuffer, size_t bufferSize) {
    printf("Enter operation (or 'exit'): ");  // Prompt user
    fgets(inputBuffer, bufferSize, stdin);     // Read input line from user
    inputBuffer[strcspn(inputBuffer, "\n")] = '\0'; // Remove newline character if present
    lastActivityTime = time(NULL);             // Update last activity time (to reset inactivity timer)
}
...
// Displays the received response or an error message
void display_response(const char *response) {
    if (strncmp(response, "ERROR", 5) == 0) {
        // Arduino returned an error
        printf("Invalid input! Please follow the correct format.\n");
        show_help_text(); // Show help again
    } else {
